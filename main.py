# -*- coding: utf-8 -*-
"""ECE602 Project Model Recreation.ipynb
...
def load_data(excel_file):
    ...
    f = 1.0 #Energy constraint; set f=1 as default
...
#%% Defining Model Constraints
    ...
    def max_capacity_rule(m_, i):
        Max_Capacity = 1000000 #tonnes of feedstock per year, capacity of digester
    ...
    def energy_eq_rule(m_):
        ...
        energy_m = sum(PlaceH * m_.MCE * m_.En_cont_CH4 * (m_.M_moved[i, j] * m_.M_CH4_potential[i]) * m_.Alpha[j] for (i, j) in m_.FEAS_M)
        energy_fw = sum(PlaceH * m_.MCE * m_.En_cont_CH4 * (m_.FW_moved[i,j] * m_.DM_FW * m_.VS_off_farm * m_.BGy_off_farm * m_.CH4c_off_farm) * m_.Alpha[j] for (i, j) in m_.FEAS_FW)
...
def solve_and_output(model, out_file = "Solution_cost_min.xlsx"):
    ...
    #If we use CPLEX_Studio as the solver (through Pyomo - Pyomo is the front end, uses Python; then it employs a solver in the backend, CPLEX
    # solver_path = r"C:\Program Files\IBM\ILOG\CPLEX_Studio2211\workspacecplex\bin\x64_win64\cplex.exe"
